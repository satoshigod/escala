...
export async function verificarCredenciales() {
  // Verificamos el token inspeccionándolo directamente via debug_token
  // No requiere pages_read_engagement
  const appId = process.env.META_APP_ID
  const appSecret = process.env.META_APP_SECRET
  const tk = token()
  const data = await graphFetch(
    `debug_token?input_token=${tk}&access_token=${appId}|${appSecret}`
  )
  ...
  return {
    ok: true,
    nombre: 'Escala Network',
    id: pageId(),
    scopes: data.data.scopes,
    expira: data.data.expires_at,
  }
}
