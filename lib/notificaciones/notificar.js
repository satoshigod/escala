...
export async function notificar(tipo, destinatarios, datos) {
  ...
  await Promise.all(lista.map(async (dest) => {
    ...
    let prefs = { email_activo: true, push_activo: true, categorias_email_desactivadas: [], categorias_push_desactivadas: [] }
    ...
    // Verificar si la categoría del evento está desactivada individualmente
    const categoriaEvento = evento.categoria || evento.modulo
    const emailCategoriaBloqueada = (prefs.categorias_email_desactivadas || []).includes(categoriaEvento)
    const pushCategoriaBloqueada = (prefs.categorias_push_desactivadas || []).includes(categoriaEvento)
    ...
    // 2. Email
    if (dest.email && evento.canales.includes('email') && (esCritica || (prefs.email_activo && !emailCategoriaBloqueada))) {
      try {
    ...
    if (dest.id && evento.canales.includes('push') && (esCritica || (prefs.push_activo && !pushCategoriaBloqueada))) {
      try {
