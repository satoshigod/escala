...
export const PLANTILLAS_EMAIL = {
  ...
  // --- nuevas para Fase 17 ---
  cumplimiento_confirmado: ({ postulante_nombre, proyecto_nombre, forma_pago_label, cumplio, workspace_url }) => ({
    subject: cumplio ? 'Tu cumplimiento fue confirmado en ' + proyecto_nombre : 'Actualización sobre tu aporte en ' + proyecto_nombre,
    html: cumplio
      ? '<div style="' + style + '">' + logo + eyebrow('#1D9E75', 'Cumplimiento confirmado') + '<h1 style="font-size:1.2rem;font-weight:800;margin:0 0 1rem">Hola ' + postulante_nombre + ',</h1><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1.5rem">Se confirmó que cumpliste tu aporte en <strong>' + proyecto_nombre + '</strong>. Forma de pago acordada: <strong style="color:#1D9E75">' + forma_pago_label + '</strong>.</p>' + btn(workspace_url, 'Ver detalle en el workspace') + '</div>'
      : '<div style="' + style + '">' + logo + eyebrow('#D85A30', 'Actualización') + '<h1 style="font-size:1.2rem;font-weight:800;margin:0 0 1rem">Hola ' + postulante_nombre + ',</h1><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1.5rem">El proyecto <strong>' + proyecto_nombre + '</strong> determinó que tu aporte no cumplió lo pactado. No aplica pago en este caso.</p>' + btn(workspace_url, 'Ver detalle en el workspace') + '</div>'
  }),
  verificar_correo: ({ nombre, verificar_url }) => ({
    subject: 'Confirma tu correo en Escala',
    html: '<div style="' + style + '">' + logo + eyebrow('#1D9E75', 'Un paso más') + '<h1 style="font-size:1.2rem;font-weight:800;margin:0 0 1rem">Hola ' + (nombre || 'bienvenido') + ',</h1><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1.5rem">Gracias por registrarte en Escala. Confirma tu correo para verificar tu cuenta — puedes seguir usando la plataforma mientras tanto, no es obligatorio para empezar.</p>' + btn(verificar_url, 'Confirmar mi correo') + '</div>'
  }),
  ...
  incentivo_cert_contador: ({ nombre, perfil_url }) => ({
    subject: '🪪 Sube tus documentos y consigue más propuestas en Escala',
    html: '<div style="' + style + '">' + logo + eyebrow('#4A90D9', '🏅 Sube tu nivel') + '<h1 style="font-size:1.2rem;font-weight:800;margin:0 0 1rem">Hola ' + (nombre || 'contador') + ',</h1><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1rem">Los fundadores de proyectos más serios buscan contadores verificados. Subir estos dos documentos <strong style="color:#4A90D9">sube tu Escala Score</strong> y aumenta directamente tus chances de recibir propuestas:</p><ul style="color:#C8D4E8;line-height:2;margin:0 0 1.5rem;padding-left:1.25rem"><li><strong>🪪 Tarjeta Profesional de Contador Público</strong> — anverso y reverso legible</li><li><strong>📋 Certificado de Vigencia JCC</strong> — expedido por la Junta Central de Contadores</li></ul><p style="color:#8FA3CC;font-size:0.82rem;margin-bottom:1.5rem">Empresas más formales los piden siempre. Subirlos te diferencia y te pone primero en la lista cuando un fundador filtra por contadores verificados.</p>' + btn(perfil_url, 'Subir mis documentos') + '</div>'
  }),
