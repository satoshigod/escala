...
export async function reportarPago(ordenId, pagadorId, ref_pago_pagador = null) {
  ...
  await notificar('admin_custodia_por_verificar', await adminParaNotificar(), {
...
export async function confirmarRecepcionReceptor(ordenId, quienConfirmaId, esAdmin = false) {
  ...
  if (orden.tipo_flujo === 'arriendo' && orden.referencia_id) {
    // Escala ya pago el deposito/arriendo al arrendador: el local queda operando
    try {
      await supabase
      ...
      console.error('custodia efecto arriendo:', e.message)
    }
  }

  if (orden.tipo_flujo === 'reparto' && orden.referencia_id) {
    try {
      await supabase
        .from('reparto_lineas')
      ...
      console.error('custodia efecto reparto:', e.message)
    }
  }

  if (orden.tipo_flujo === 'local_repago' && orden.proyecto_id) {
    try {
      // los montos del waterfall se leen del reporte diario (columnas reales,
      // verificado contra el esquema: intereses_dia y abono_capital existen)
      const { data: rep } = await supabase
        .from('reportes_diarios_local')
        .select('intereses_dia, abono_capital')
        .eq('id', orden.referencia_id)
        .maybeSingle()
      ...
      if (rep) {
        const { data: local } = await supabase
          .from('proyectos_local_comercial')
          .select('id, capital_pagado, intereses_pagados, capital_total, fase_actual')
        ...
        if (local) {
          ...
          const saldo = (local.capital_total || 0) - nuevoCapital
          await supabase
            .from('proyectos_local_comercial')
            .update({
...
export async function cancelarOrden(ordenId, quienId, motivo = null) {
  ...
  const { data, error } = await supabase
    ...
    .update({
      estado: 'cancelado',
      // no se pisa `nota`: puede llevar los datos del flujo (waterfall).
      // El motivo se anexa como referencia administrativa.
      ref_pago_escala: motivo ? `CANCELADA: ${motivo}` : 'CANCELADA',
      updated_at: new Date().toISOString(),
    })
    .eq('id', ordenId)
