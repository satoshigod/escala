  ...
  async function probar() {
    ...
    try {
      const res = await fetch('/api/paises', {
        ...
        body: JSON.stringify({ nombre: nombre || 'PaisDePrueba' + Date.now(), bandera: '🧪', tipo_origen: 'diagnostico' })
      })
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0D1B3E',fontFamily:'Inter,sans-serif',padding:'2rem',color:'#fff'}}>
      <div style={{maxWidth:'600px',margin:'0 auto'}}>
        <h1 style={{fontSize:'1.5rem',fontWeight:'800',marginBottom:'0.5rem'}}>Diagnóstico — Crear país</h1>
        <p style={{color:'#8FA3CC',marginBottom:'2rem'}}>Esta página prueba directamente la API sin pasar por el formulario normal.</p>

        <label style={{display:'block',fontSize:'0.8rem',color:'#8FA3CC',marginBottom:'0.5rem'}}>Nombre del país a probar</label>
        <input
          value={nombre}
        ...
        >
          {cargando ? 'Probando...' : 'PROBAR CREAR PAÍS'}
        </button>
