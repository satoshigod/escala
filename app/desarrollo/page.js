...
const CAPAS = [
  {
    ...
    hitos: [
      ...
      { num: 'C0.5', nombre: 'Componentes base reutilizables. HECHO: se crearon los 4 componentes mas repetidos en components/base.js (Card, Pill, EmptyState, Modal), extraidos del diseno real ya existente, con el objeto TONOS centralizando la paleta (semilla de C0.9). Migradas 24 paginas de la app funcional: el nucleo (dashboard, proyectos, workspace + sus 11 sub-paginas) y los paneles (angel, mis-contratos, postulaciones, score, admin-escala, aportes, carril, hitos, invitar, mi-equipo, mi-cartera, ingresos, comercial). Galeria de referencia en /qa-componentes. Se migro por criterio semantico, no por coincidencia de estilo (L12): quedaron fuera a proposito los botones (un componente de solo-lectura los romperia), los contenedores con layout propio, y las landings de marketing (lenguaje visual distinto, con su propio sistema; L17). El objetivo del refactor era que el patron del producto salga de un componente y no este duplicado — cumplido.', done: true, valor: 4000000, quien: 'Claude AI' },
      { num: 'C0.6', nombre: 'Tests unitarios del motor financiero. HECHO: se extrajo la logica financiera pura a lib/financiero/calculos.js (sin dependencia de Supabase) y se cubrio con 29 tests que corren con el runner nativo de Node, sin base ni credenciales: saldo del ledger (creditos-debitos), doble partida (2 entradas, sufijos _db/_cr, columna real referencia_tipo), conversion USD, comision, waterfall del local comercial (dia normal, excedente que no cubre intereses, deficit, tope del saldo, transicion repago->regalia, regalia sobre ventas brutas) y maquina de estados de custodia (flujo feliz, no saltar pasos, reportar no es recibir, cancelacion solo antes de emitir). No toca ledger.js ni custodia.js de produccion — evita el riesgo de refactorizar el motor sin red; estos tests SON la red para C0.10. Enchufados al workflow build: corren en cada push antes de compilar. 29/29 verde.', done: true, valor: 2500000, quien: 'Claude AI' },
      ...
      { num: 'C0.8', nombre: 'Consultas preparadas para escala. HECHO PARCIAL: se agregaron 16 indices en las 8 tablas de mayor trafico — proyectos, perfiles y tareas solo tenian su primary key, y como las politicas RLS filtran por fundador_id, estado y proyecto_id y RLS corre en CADA consulta, cada lectura escaneaba la tabla entera. PENDIENTE: 236 selects sin limite y 23 con select(*) que traen columnas de mas. Con 9 usuarios no importa; con 10.000 proyectos una consulta sin limite tumba la pagina. Agregar limites y paginacion en los listados, y pedir solo las columnas que se usan.', done: false, valor: 2500000, quien: 'Claude AI' },
      { num: 'C0.9', nombre: 'Design tokens. SISTEMA HECHO, migracion progresiva: se creo lib/tokens.js como fuente unica de verdad de la paleta — TONO (tonos de marca con hex+rgb), COLOR (colores semanticos de texto/superficie/borde) y el helper tono(nombre, alpha) que construye los rgba tenues (el patron mas repetido). base.js ya importa de tokens en vez de duplicar. Cambiar la paleta ahora es tocar UN archivo. 7 tests de tokens. PENDIENTE: reemplazar los ~2500 colores hardcodeados en los 116 archivos (ej. #8FA3CC ~1200 veces, #1D9E75 ~770) por referencias al token — es un reemplazo masivo tipo C0.5 que se hace por partes; el codigo nuevo ya usa tokens, el viejo se migra progresivamente.', done: false, valor: 2000000, quien: 'Claude AI' },
  ...
  },
  {
    ...
    hitos: [
      ...
      { num: 'C3.30', nombre: 'CUSTODIA: flujo compra_maquina cableado. Cuando el inversionista firma el contrato de leasing y queda activo, nace una orden de custodia: el inversionista le paga a Escala el valor del equipo, Escala custodia ese dinero y compra la maquina al proveedor. El receptor es externo (proveedor/distribuidor) por lo que receptor_id va null con receptor_externo descriptivo, y el tramo final lo confirma el admin. Idempotente por contrato (custodia-leasing-{id}). Falta replicar a: arriendo, fondeo, reparto.', done: true, valor: 800000, quien: 'Claude AI' },
      { num: 'C3.31', nombre: 'CUSTODIA: flujos fondeo y reparto cableados. FONDEO: al aceptar el fundador nace la orden (inversionista paga a Escala); el ledger de acreditacion al proyecto ahora sale de escala:custodia en vez de angel:X directo. REPARTO: marcar una linea ya no la deja "pagado" ni escribe ledger directo proyecto->perfil; la linea pasa a "en_pago", nace la orden (proyecto -> Escala -> beneficiario) y solo queda "pagado" cuando el beneficiario confirma que recibio. La comision Escala 3% se mantiene como cobro de plataforma. HALLAZGO: el flujo "arriendo" NO se pudo cablear porque todavia no existe el endpoint donde un inversionista acepta financiar un local (el local se aprueba con tasa pero nunca se asigna inversionista_id); cablearlo requiere crear ese flujo primero.', done: true, valor: 1200000, quien: 'Claude AI' },
      { num: 'C3.32', nombre: 'Flujo faltante: el inversionista puede financiar un local (hueco real del producto, no solo de custodia). Antes el local se aprobaba con tasa pero NUNCA se le asignaba inversionista: /api/inversiones/oportunidades solo leia presupuesto_items, asi que los locales aprobados no aparecian en ningun lado. Ahora: (1) los locales aprobados sin inversionista se exponen como oportunidad, (2) nuevo endpoint /api/local-comercial/financiar (GET lista, POST el inversionista lo toma con guard optimista anti-carrera, valida que no sea su propio local, asigna inversionista_id, pasa a esperando_capital y crea la orden de CUSTODIA del arriendo inversionista -> Escala -> arrendador), (3) notificacion al tendero de que ya tiene inversionista. Con esto el flujo arriendo queda cableado a custodia (era el que faltaba en C3.31).', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C3.33', nombre: 'Directorio de inversion agrupado por tipo de oportunidad (pedido de Ivan: locales, equipos y demas separados, no en una lista mezclada). Las oportunidades se clasifican en 6 secciones (Locales comerciales, Equipos y maquinaria, Talento y nomina, Tecnologia, Inventario e insumos, Otras necesidades) y cada seccion muestra su conteo; las secciones vacias no se renderizan. El boton enruta segun el tipo: los locales llaman al endpoint de financiar local, los items de presupuesto abren el modal de terminos. Corregido de paso un bug de orden de declaracion (localesComoItems se usaba antes de declararse -> ReferenceError).', done: true, valor: 700000, quien: 'Claude AI + Ivan' },
      { num: 'C3.34', nombre: 'Revision estatica de la cadena de custodia (no se puede probar en vivo desde el entorno: la red bloquea Supabase y escala.network). Verificado OK: columnas de ordenes_pago usadas por el codigo existen todas en el SQL; los 10 params que custodia pasa a registrarMovimiento coinciden con su firma; los guards optimistas de cada transicion son correctos; las idempotency_key de los dos tramos (custodia_in_/custodia_out_) no colisionan. BUGS ENCONTRADOS Y CORREGIDOS: (1) custodia leia local.capital_original como columna, pero capital_original es una variable calculada en el reporte -- la columna real es capital_total; con el bug el cambio de fase a regalia nunca habria ocurrido. (2) cancelarOrden pisaba la columna nota, que en el flujo local lleva los datos del waterfall; ahora el motivo va aparte y no destruye datos.', done: true, valor: 400000, quien: 'Claude AI' },
      { num: 'C3.35', nombre: 'Esquema de local comercial completado (verificado contra la BD real: faltaban 3 columnas). La tabla proyectos_local_comercial NO tenia operador_id, inversionista_id ni estado — el codigo nuevo de financiar local las usaba, y el panel del inversionista (/api/local-comercial/inversionista) ya estaba ROTO desde antes filtrando por .eq(estado, activo) sobre una columna inexistente, por eso siempre salia vacio. SQL corrido: se agregan las 3 columnas con FK a perfiles, CHECK de estados (borrador / buscando_inversionista / esperando_capital / activo / cerrado), backfill de operador_id desde proyectos.fundador_id e indices. Ciclo de vida cableado: aprobar -> buscando_inversionista (aparece en el directorio), inversionista lo toma -> esperando_capital, orden de custodia del arriendo completada (Escala pago al arrendador) -> activo.', done: true, valor: 600000, quien: 'Claude AI + Ivan' },
      { num: 'C3.36', nombre: 'Auditoria directa contra la BD (conector Supabase). BUG CRITICO CORREGIDO: reparto_lineas tenia CHECK que solo permitia pendiente/pagado, pero el modelo de custodia escribe el estado intermedio en_pago -> el flujo de reparto habria fallado con violacion de constraint en produccion. Migracion aplicada: el CHECK ahora acepta pendiente/en_pago/pagado. Tambien corregido: contratos_leasing no tiene columna numero sino numero_contrato (el concepto de la orden mostraba el uuid en vez del numero de contrato). SIMPLIFICACION: reportes_diarios_local SI persiste intereses_dia/abono_capital/pago_inversionista, asi que se elimino el hack de guardar el waterfall serializado en la columna nota; custodia ahora lee las columnas reales.', done: true, valor: 700000, quien: 'Claude AI' },
      { num: 'C3.38', nombre: 'PRUEBA END-TO-END del flujo de local con custodia (ejecutada contra la BD real con datos QA, limpiados al final). Los 6 pasos pasaron: crear local -> admin aprueba (buscando_inversionista) -> aparece como oportunidad -> inversionista lo toma (esperando_capital + orden de custodia) -> Ya pague -> admin confirma recepcion (LEDGER TRAMO 1: perfil -> escala:custodia) -> Escala emite -> receptor confirma (LEDGER TRAMO 2: escala:custodia -> pagos_externos) -> local activo. Saldo de custodia cuadra en 0. BUGS CRITICOS QUE DESTAPO LA PRUEBA: (1) proyectos_escenario_check NO aceptaba maquinaria (solo startup/local_comercial/otro), asi que crear un proyecto de maquinaria habria fallado en produccion tras la migracion del hallazgo #8 -- migracion aplicada. (2) 10 inserts directos a ledger_entries en 6 APIs usaban la columna tipo_referencia cuando la real es referencia_tipo, y ademas ninguno pasaba tasa_usd que es NOT NULL: TODA escritura directa al ledger fallaba, y varias con .catch() vacio fallaban en silencio (por eso ledger_entries tenia 0 filas). Corregidos los 10. El motor registrarMovimiento si estaba bien.', done: true, valor: 1200000, quien: 'Claude AI' },
      { num: 'C3.39', nombre: 'PRUEBA E2E de TODOS los flujos contra la BD real (maquinaria, startup+fondeo, reparto, local), datos QA limpiados al final. Balance de doble partida verificado: la suma de todas las cuentas da 0. Se probo: proyecto escenario maquinaria + contrato leasing + orden compra_maquina (ciclo completo); proyecto startup + presupuesto_items + presupuesto_fondeos + orden fondeo + comision Escala; reparto con linea en_pago -> pagado. BUGS ADICIONALES DEL LEDGER DESTAPADOS: (3) los inserts pasaban comision_escala como columna pero NO existe en ledger_entries (movido a la columna metadata jsonb, 5 sitios). (4) tipo: comision es INVALIDO — tipo es un enum tipo_ledger que solo acepta debito/credito, asi que TODOS los asientos de comision fallaban (corregidos a debito en 5 sitios; referencia_tipo ya identifica que es comision). Sumado a los 2 bugs previos (referencia_tipo y tasa_usd), ninguna comision de Escala se habia registrado nunca. Hallazgo menor: presupuesto_items.valor_total es columna generada (cantidad * valor_unitario), el codigo ya lo respeta.', done: true, valor: 900000, quien: 'Claude AI' },
      { num: 'C3.40', nombre: 'Higiene de SQL: carpeta sql/ organizada con las 4 migraciones nombradas (nunca "untitled query"), formato AAAA-MM-DD_descripcion_snake_case.sql, cada una con cabecera explicando que arregla y como se aplico. README con convenciones: DDL siempre idempotente (IF NOT EXISTS / DROP CONSTRAINT IF EXISTS), no borrar datos sin respaldo, y no renombrar valores tecnicos sin migracion + periodo de compatibilidad. Las 2 migraciones aplicadas via conector quedaron registradas en supabase_migrations.schema_migrations con nombre; las 2 que se corrieron a mano en el editor no quedan en ese historial, por eso se documentan aqui.', done: true, valor: 300000, quien: 'Claude AI + Ivan' },
      { num: 'C3.41', nombre: 'Auditoria COMPLETA de la base (42 tablas). BUG DE PRODUCCION CORREGIDO: perfiles_rol_principal_check solo aceptaba 5 roles (ideador, capitalista, especialista, ejecutor, angel) pero el onboarding y /bienvenida ofrecen 6 incluyendo Mentor y Empresa — quien elegia esos dos NO podia guardar su perfil. Migracion aplicada (se conserva angel por datos historicos). VERIFICADO OK: los 36 CHECK constraints contra los valores que escribe el codigo; integridad referencial sin huerfanos en 11 relaciones; datos consistentes (0 escenarios otro, 0 perfiles angel/capitalista). RLS CERRADO: costos_predefinidos (catalogo, lectura publica) y costos_proyecto (solo fundador o postulacion aceptada) ya tienen RLS — las 42 tablas cubiertas. SEGURIDAD CERRADA: los 4 ERRORs del linter resueltos (RLS de costos + las 2 vistas de saldos wallet_balances/wallet_comprometido pasadas a security_invoker: eran SECURITY DEFINER y exponian el saldo de TODOS los usuarios), search_path fijo en las 7 funciones, y permisos de ejecucion revocados de PUBLIC/anon/authenticated en las 4 funciones de trigger + crear_wallet_si_no_existe (era SECURITY DEFINER y creaba wallets sin login) + calcular_escala_score; service_role conserva acceso. HALLAZGO: el QA llama a eliminar_usuario_completo() que NO EXISTE en la base (falla en silencio por .catch vacio), esa limpieza nunca funciono. POLITICAS PERMISIVAS CERRADAS: las 5 con USING(true) reemplazadas — tareas ahora exige pertenencia al equipo (fundador o postulacion aceptada) porque el cliente si escribe desde workspace/constitucion; en logros_usuario, historial_tareas y documentos_proyecto se quitaron (solo se escriben por API con service role). STORAGE CERRADO: el bucket evidencias tenia SELECT amplio (listar todos los archivos) e INSERT abierto (cualquier anonimo podia subir); ambas quitadas, las subidas pasan por /api/upload con service role. El linter paso de ~28 hallazgos a 3, sin ningun ERROR. Los 3 restantes no son problema: impulsos y postulaciones tienen RLS sin politicas (quedan cerradas al cliente, se usan solo por API) y la proteccion de contrasenas filtradas (HaveIBeenPwned) NO esta disponible en el plan gratuito de Supabase — queda descartada, no es un agujero sino una capa extra opcional.', done: true, valor: 800000, quien: 'Claude AI + Ivan' },
      { num: 'C3.37', nombre: 'SEGURIDAD (pendiente de decision): costos_proyecto (19 filas) y costos_predefinidos (19 filas) tienen RLS DESACTIVADO — cualquiera con la anon key publica puede leer o modificar todas sus filas. Son las 2 unicas tablas sin RLS de 42. NO se auto-corrige: activar RLS sin politicas bloquearia el acceso y rompe la app, hay que definir las politicas primero. Hallazgo menor: impulsos y postulaciones tienen RLS activo pero 0 politicas (bloqueadas para el cliente; funcionan solo via service role).', done: false, valor: 300000, quien: 'Claude AI + Ivan' },
    ]
  },
  {
    id: 'C4',
  ...
  },
  {
    ...
    hitos: [
      ...
      { num: 'C11.19', nombre: 'PROGRAMA 10 MAQUINAS - solicitud publica. Vacio critico: las landings existian pero NO habia donde aplicar (el CTA iba a wa.me/573000000000, un numero de relleno: cualquiera que hiciera clic caia en la nada). Nueva pantalla /programa/aplicar con captura progresiva en 3 pasos (que necesitas / que produces hoy / quien eres). No pide documentos: eso viene tras la preaprobacion, cuando ya hay compromiso mutuo. Al enviar, la persona ve su plan estimado (abono mensual y rango de meses). Las 3 landings ahora apuntan alli, y ademas capturan los datos del scoring, cosa que WhatsApp no hacia. Tabla solicitudes_programa con maquina de estados y RLS.', done: true, valor: 2000000, quien: 'Claude AI' },
      { num: 'C11.20', nombre: 'PROGRAMA - motor de scoring de credito (lib/programa/scoring.js). El escala_score existente NO servia: mide reputacion dentro de la plataforma (tareas cumplidas), no capacidad de pago de alguien que llega por un anuncio y nunca la ha usado. Nuevo modelo con descartes duros (sin produccion, fuera del Valle de Aburra, menos de 6 meses de oficio) y 5 variables ponderadas: experiencia 25, produccion semanal 25, estabilidad del cliente 20, ingreso vs valor del equipo 20, infraestructura 10. Tres bandas: alta >=75 preaprueba sola, media >=50 va a analista, baja rechaza con mensaje digno y ruta alterna. Umbrales conservadores a proposito: preaprobar de mas con capital ajeno es el error caro.', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C11.21', nombre: 'PROGRAMA - bandeja de revision (/admin/programa). Sin esto nadie podia aprobar. Muestra la cola con el score y el detalle de que llevo arriba y que llevo abajo a cada solicitud, el plan estimado, y las acciones preaprobar / rechazar / verificar / cotizar. Las transiciones se validan en el backend (no se puede saltar pasos). KPIs de la bandeja: por revisar, preaprobadas, verificadas y maquinas colocadas sobre 10.', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C11.22', nombre: 'PROGRAMA - entrega e instalacion (/api/programa/entrega). Antes el contrato pasaba a activo y ahi se perdia el rastro: no habia forma de saber si la maquina llego. Ahora se registra la cadena fisica completa con estados nuevos del contrato: activo -> equipo_comprado -> entregado -> en_operacion. Guarda proveedor, serial real, quien instalo, quien capacito y firma de recibido. Tablas proveedores_equipo (con los 9 distribuidores de C11.18 sembrados) y entregas_equipo.', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C11.23', nombre: 'PROGRAMA - gestion de mora (/api/programa/mora). La clausula del contrato ("si no reportas 3 meses seguidos") existia pero no habia proceso que la ejecutara: era letra muerta. Escalera progresiva por dias sin reportar: 35 recordatorio automatico, 50 llamada de operacion, 65 marca en_mora y avisa al inversionista, 95 se ofrece reestructuracion, 125 el comite evalua retiro del equipo. DISTINCION CLAVE DEL MODELO: reportar POCO no es mora (es exactamente lo que el producto promete); la mora es NO REPORTAR. La reestructuracion baja el % del excedente pero NUNCA condona capital, porque el capital es del inversionista, no de Escala.', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C11.24', nombre: 'PROGRAMA - paneles de los 3 actores. /mi-equipo: panel de quien tiene un equipo en leasing, GENERICO para cualquier activo (maquina, freidora, secadora, vehiculo), pensado para celular. Muestra cuanto le falta, el progreso, en que punto va la entrega y el boton de reportar, con texto de ayuda en lenguaje llano por estado. /mi-cartera: panel del inversionista con invertido, recuperado, equipos en riesgo y el detalle de cada uno; cuando hay mora se le aclara que el equipo sigue siendo suyo hasta recuperar el capital (es la garantia real del modelo y hay que hacerla visible justo cuando se preocupa). /admin/programa/operacion: las 10 maquinas de un vistazo con entrega, mora y KPIs.', done: true, valor: 2000000, quien: 'Claude AI' },
      { num: 'C11.25', nombre: 'PROGRAMA - carta de liberacion (/api/programa/liberacion). Es el momento de mayor valor del modelo: la persona empezo sin poder comprar el equipo y termina siendo dueña; merecia un documento formal, no un cambio de estado silencioso. Genera la carta con los datos reales del contrato y 5 declaraciones, incluida que el historial de cumplimiento queda como antecedente favorable para futuras operaciones. Solo se emite si el capital pendiente llega a cero (validado en backend) y la emite un humano aunque el calculo sea automatico, porque certifica una transferencia de propiedad. Notifica a ambas partes con el mensaje que cada una necesita.', done: true, valor: 1500000, quien: 'Claude AI' },
      { num: 'C11.26', nombre: 'PROGRAMA - KPIs del piloto (/api/programa/kpis). En un piloto de 10 maquinas el objetivo no es volumen, es APRENDER, asi que cada metrica responde una pregunta que hay que contestar antes de escalar: embudo de conversion paso a paso (donde se cae la gente), calidad del scoring por banda (si la alta y la media se comportan igual, el modelo no separa y hay que ajustarlo), tasa de mora sobre CAPITAL y no sobre numero de contratos (es la que le importa al inversionista), cumplimiento del reporte mensual (el habito que sostiene el modelo) y tasa de calificacion por origen de campana (que anuncio trae a los que si califican, no solo a los que mas clic hacen).', done: true, valor: 1000000, quien: 'Claude AI' },
      { num: 'C11.27', nombre: 'PROGRAMA - pendientes antes de lanzar la campana: (1) HABEAS DATA en el formulario de solicitud, que recoge cedula y celular sin casilla de aceptacion ni politica visible (requisito legal en Colombia), (2) revision del contrato de leasing por abogado antes de las primeras firmas, (3) probar el flujo completo con una solicitud real de punta a punta. Riesgos documentados: el subreporte de ventas es estructural al modelo y su mejor control no es tecnico sino el incentivo (mientras mas rapido paga, mas rapido el equipo es suyo); el riesgo legal serio es que la operacion se interprete como intermediacion financiera sin licencia, requiere concepto antes de escalar mas alla del piloto.', done: false, valor: 1000000, quien: 'Ivan' },
    ]
  },
]
...
const LECCIONES = [
  ...
  },
  {
    ...
    regla: 'Verificar que compila (L1) es necesario pero no suficiente: hay una clase entera de errores (orden de declaracion, variables inexistentes, relaciones de datos ambiguas) que solo aparece en ejecucion. Pero decir "hay que abrir la pagina en el navegador" no es una mitigacion real si no hay nadie ni nada que lo haga de forma sistematica. En un proyecto de una sola persona, los deploys son automaticos y nadie navega cada pagina despues de cada uno: un bug de runtime puede vivir en produccion hasta que un usuario tropieza con el. La verificacion post-deploy tiene que estar AUTOMATIZADA, no depender de la disciplina de acordarse de revisar. Toda pagina critica necesita una prueba de humo que la cargue de verdad (navegador headless) y golpee sus APIs con datos reales, corriendo sola tras cada deploy. Sin eso, "verificar en runtime" es una buena intencion que nadie ejecuta.',
    estado: 'abierto',
  ...
  },
  {
    ...
    categoria: 'Verificacion',
    correcciones: 0,
    titulo: 'Para testear logica acoplada a la base, separar la formula del acceso',
    caso: 'El motor financiero (ledger, custodia) creaba el cliente de base de datos a nivel de modulo, asi que sus funciones no se podian probar en aislamiento: importarlas ya intentaba conectar. Testearlas habria exigido una base real o reescribir el motor — y reescribir el modulo mas delicado SIN tener tests todavia es justo el riesgo que los tests buscan eliminar (circulo vicioso). La salida fue extraer las formulas puras (waterfall, doble partida, saldo, transiciones de estado) a un archivo aparte que recibe numeros y devuelve numeros, sin tocar la base, y probar ESO. 29 tests que corren en milisegundos sin credenciales, y el motor de produccion quedo intacto.',
    regla: 'La logica de negocio (el calculo) y el acceso a datos (leer/escribir la base) son dos cosas distintas y deben poder separarse. Cuando estan mezcladas en la misma funcion, esa funcion no se puede testear sin la base. La formula pura — la parte que solo depende de sus entradas — se extrae a funciones sin dependencias y se cubre con tests rapidos. Esto se puede hacer SIN tocar el codigo de produccion (una copia fiel de la formula que sirve de red), evitando el circulo vicioso de tener que refactorizar lo delicado antes de tener con que verificarlo. Idealmente el codigo nace ya separado; cuando no, esta es la forma segura de ponerle red.',
    estado: 'corregido',
    mitigacion: 'C0.6: lib/financiero/calculos.js con las formulas puras y 29 tests. El ledger de produccion no se toco; los tests son la red para cuando se refactorice (C0.10).',
  },
  {
    id: 'L19',
    categoria: 'Arquitectura',
    correcciones: 0,
    titulo: 'Centralizar no es lo mismo que migrar: un token es el lugar, no el reemplazo',
    caso: 'C0.9 (design tokens) parecia que consistia en eliminar los ~2500 colores hardcodeados. Pero reemplazarlos todos de golpe en 116 archivos es un trabajo masivo y arriesgado. Lo que de verdad resuelve el problema (cambiar la paleta sin buscar-y-reemplazar en todo el codigo) es crear la fuente unica de verdad — el archivo de tokens — y que el codigo nuevo la use. Los colores viejos siguen ahi, pero ya existe el lugar correcto donde centralizarlos, y la migracion es progresiva. Confundir "crear el sistema" con "migrar todo" habria hecho parecer C0.9 imposible o incompleto.',
    regla: 'Crear el mecanismo central (tokens, componentes, un helper) y migrar todo el codigo viejo a el son dos trabajos distintos, y el primero entrega la mayor parte del valor. Con los tokens creados, cambiar la paleta ya es tocar un archivo, aunque no se haya migrado ni una linea vieja. La migracion masiva es incremental y de bajo riesgo si se hace por partes; el codigo nuevo adopta el mecanismo desde el dia uno y el viejo se va sumando. No hay que bloquear el valor del sistema esperando a reemplazarlo todo.',
    estado: 'en_proceso',
    mitigacion: 'C0.9: lib/tokens.js creado como fuente unica, base.js ya lo usa. Reemplazo de los colores hardcodeados pendiente y progresivo — el codigo nuevo usa tokens, el viejo se migra por partes.',
  },
]
