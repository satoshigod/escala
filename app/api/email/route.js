...
const templates = {
  ...
  nuevo_pais: ({ pais_nombre, bandera, creado_por, tipo_origen, admin_url }) => ({
    ...
    html: '<div style="' + style + '">' + logo + '<div style="font-size:0.75rem;font-weight:700;color:#E8A020;margin-bottom:0.5rem;text-transform:uppercase">⚠️ Acción requerida</div><h1 style="font-size:1.2rem;font-weight:800;margin:0 0 1rem">' + (bandera||'🌐') + ' ' + pais_nombre + ' fue agregado</h1><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1.5rem">Un ' + tipo_origen + ' creó el país <strong style="color:#E8A020">' + pais_nombre + '</strong> en la plataforma. Este país todavía <strong>no tiene tareas regulatorias configuradas</strong>.</p><p style="color:#C8D4E8;line-height:1.7;margin-bottom:1.5rem">Entra al panel admin y agrégalas para que los próximos proyectos en ' + pais_nombre + ' las reciban automáticamente.</p>' + btn(admin_url, 'Configurar tareas para ' + pais_nombre) + '<p style="color:#8FA3CC;font-size:0.82rem">Creado por: ' + creado_por + '</p></div>'
  }),
